...
namespace MechTE_480.Util
{
    ...
    /// 加载DLL
    /// </summary>
    public static class LoadDllUtil
    {
        //已加载的DLL
        private static readonly Dictionary<string,Assembly> SLoadedDlls = new Dictionary<string,Assembly>();
        //已处理的程序集
        private static readonly Dictionary<string,object> SAssemblies = new Dictionary<string,object>();
        ...
        private static Assembly AssemblyResolve(object sender,ResolveEventArgs args)
        {
            ...
            if (SLoadedDlls.TryGetValue(assName,out Assembly ass) && ass != null)
            {
                SLoadedDlls[assName] = null;
                return ass;
            } else
            {
                //抛出加载失败的异常
        ...
        public static void RegistDll()
        {
            ...
            if (SAssemblies.ContainsKey(ass.FullName))
                return;
            else
                SAssemblies.Add(ass.FullName,null);
            //绑定程序集加载失败事件
            ...
            foreach (string res in resources)
            {
                ...
                if (regex.IsMatch(res))
                {
                    ...
                    //判断是否已经加载
                    if (SLoadedDlls.ContainsKey(loadedAss.FullName))
                        continue;
                    else
                        SLoadedDlls[loadedAss.FullName] = loadedAss;
                }
            }
        }
    }
